...
RUN apk add --no-cache make sshpass openssh gcc g++ libffi-dev openssl openssl-dev \
...
COPY --from=build /usr/local/bin/ansible \
    /usr/local/bin/ansible
COPY --from=build /usr/local/bin/ansible-playbook \
